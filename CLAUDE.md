...
**差し戻しは 2 回。どちらも ASC の書き漏れで、ビルドは一度も作り直していない。**
経緯と対処は `../coin-laundry-app/docs/ios/app-store/metadata_ja.md` の
「2026-08-13 の差し戻し 2 件」にある。⚠️ **次のバージョンでも同じ形で来る。**

| | |
|---|---|
| 3.1.2(c) | 利用規約（EULA）へのリンクが**メタデータに**無い。⚠️ **アプリ内には最初からあった。**アプリを直しに行くと丸ごと無駄になる |
| 2.1 | 新規アプリへの 8 項目の情報要求。⚠️ **最初から審査情報のメモに書いておけば来ない**（Apple 自身がそう書いている） |

⚠️ **デモアカウントを消さない。** 次のバージョンの審査でも同じものが使われる。

提出時に答えたアンケート（年齢制限・コンテンツ配信権・App のプライバシー・
アクセシビリティ）も**理由ごと** `metadata_ja.md` に残してある。

提出時に答えたアンケート（年齢制限・コンテンツ配信権・App のプライバシー・
アクセシビリティ）は**理由ごと** `../coin-laundry-app/docs/ios/app-store/metadata_ja.md`
に残してある。**次のバージョンでも同じことを聞かれる。**
...
指摘が来そうなところは全部確認済みなので、慌てないこと。

| 指摘されうるもの | 手元の状態 |
|---|---|
| **デモアカウントで入れない**（2.1） | ⚠️ **いちばん多い差し戻し。**起動して最初が素のログイン壁なので、入れないと審査員は 1 画面も見られない |
| アカウント削除（5.1.1(v)） | ✓ 実機確認済み。⚠️ **入口は 2 つ**（設定 / `/join-organization`） |
| 購入を復元（3.1.1） | ✓ TestFlight で確認済み |
| Apple サインイン（4.8） | ✓ 実機確認済み。`@privaterelay.appleid.com` も弾いていない |
| 外部購入への言及（3.1.3(a)） | ✓ 掲載情報・規約・お知らせ・アプリ内すべてから外してある |

⚠️ **説明不足の指摘なら再ビルドは要らない**（返信だけで済むことが多い）。
ビルドを差し替えるときは**一度提出を取り消す**ことになる。

## 提出後に回すもの

- [ ] ⚠️ **アプリを `main` にマージする。** 開発は `feat/iap-and-push`。
      **`main` には 124 コミット入っていない**
- [ ] **無料トライアル（導入オファー）。** ⚠️ **審査を通ったので欄が現れている。**
      ⚠️ **アプリ側は完成済みで再ビルド不要**（`src/billing/introOffer.ts` が
      StoreKit の返した内容をそのまま出す）。ASC で設定した瞬間にビルド 17 に出る。
      - ⚠️ **購読グループ単位で 1 回**なので、3 商品すべてに付けても二重取りされない。
        Pro だけに付けると**上位プランを選んだ人が無料期間を失う**
      - ⚠️ **自分の Apple ID では確認できない**（既に契約済み＝受給資格が無く、
        StoreKit が `introductoryOffer` を返さない）。**出ないのが正しい挙動**で、
        実装の不具合ではない。試すなら Sandbox のテストアカウント +
        **開発ビルド**（TestFlight は本物の Apple ID なので不可）
      - ⚠️ 反映まで数時間かかることがある
...
### 次のバージョンを出すときに一緒に直すもの

⚠️ **どれも 1.0 では直せない**（説明文もアプリも審査を通さないと変わらない）ので、
**次のリリースに載せる**こと。単体で審査に出す価値は無い。
...
- [ ] **アクセシビリティ栄養ラベルを「はい」にできるようにする。**
      ...
      ⚠️ **ダークインターフェイスだけは全画面の配色を作り直すことになる**ので別枠
