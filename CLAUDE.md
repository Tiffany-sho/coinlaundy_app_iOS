...
- ⚠️ **アプリに濃色の面は 1 つも無い**（2026-08-01 に未ログイン画面を廃止したため）。
  `AuthBackground` / `onDark.*` / `Button variant="light"` も消してある。**戻さない**
- ⚠️ **ログイン・新規登録は純白の紙にフォームを直接置く**（`auth/AuthScreen.tsx`）。
...
**機能はすべて実装済みで、実機での確認もほぼ通っている。残るのは審査提出の作業。**
...
| **アプリ内課金（Pro / Pro+ / Max の購入とアップグレード）** | ✓ Sandbox |
...
- [ ] ⚠️ **`5b5f8df` を載せた production ビルドを作り直す。**
      Pro の商品 ID を `com.collecie.app.pronormal.monthly` に差し替えたが、
      **商品 ID はバイナリに焼き込まれる**ので今の TestFlight ビルドは古い ID のまま
      （ASC から削除済み＝**Pro が購入できない**）
      - 作り直したら **Pro の購入だけ**再確認する（Pro+ / Max は確認済み）
- [ ] ⚠️ **「購入を復元」が未確認。** 審査で**実際に試される**（Guideline 3.1.1）。
      ⚠️ **復元では `finishTransaction()` を呼ばない**（対象は既に完了済みの取引）
- [ ] アカウント削除の動線確認（Guideline 5.1.1(v)）
- [ ] ⚠️ **特商法をアプリに載せない判断の最終確認。** 理由は「App Store 経由の購読では
      Apple が販売者になる」こと。**法務の判断なので提出前に確認する**
      （`docs/contracts.md` の「法務文書」）
- [ ] ASC で **3 商品の日本価格**（¥800 / ¥1,500 / ¥3,000）と**提出するビルド**を確認
