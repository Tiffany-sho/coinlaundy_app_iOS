...
- **経費も担当店舗で絞り、権限を分けた**（2026-08-03）。担当外の店舗の経費は
  一覧に出ない（**組織全体のぶんは残す**）。**編集・削除と固定費の管理は admin だけ**で、
  集金担当者は**単発の登録だけ**できる。⚠️ 強制は `expenses/action.js` の中
  （`getStores()` ではない）。規約は `docs/contracts.md` の「担当店舗と権限」
  - ⚠️ **出し分けは `canAdd` と `isAdmin` の 2 つ。** 1 つにまとめると
    集金担当者に押しても 403 になるボタンが出る
...
- **経費画面を 1 枚にまとめた**（2026-08-03）。`expenses/recurring` の専用ページを
  ...
  **固定費の「名前」の入力欄を外した**（カテゴリで足りる）。
  ⚠️ **送りは `ChartPager`**（月別売上と同じ）。`revenue/_layout.tsx` の
...
- [ ] 画面ひとつずつの実機確認（**まだ通しで見ていない**）。ブラウザで動いていて
      ...
      - ⚠️ **012 で入れた分も未確認**（2026-08-03）。見るところ:
        初期設定の「経費の記録」ステップ（**新しい組織を作らないと出ない**）/
        設定 → 組織 の切り替えと、**切ったときに収益タブから「月別利益」と
        経費ボタンが消えること** / **月別利益の赤字の月**（0 の線より下へ伸びる。
        経費が売上を超える月をわざと作って見る）/ 経費一覧の絞り込み
