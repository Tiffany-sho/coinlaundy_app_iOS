...
import { ASSIGNABLE_ROLES, ROLE_INFO, orgStyles } from "@/components/settings/orgShared";
  ...
  function onChangeRole(member: OrgMember) {
    void (async () => {
      const role = await dialog.choose<Role>({
        ...
        options: ASSIGNABLE_ROLES.filter((r) => r !== member.role).map((r) => ({
      ...
      >
        ...
        >
          ...
          <View style={orgStyles.card}>
            ...
            {pending.length > 0 && <JoinRequestSection requests={pending} />}
          </View>

          {!isAdmin && <Muted>役割の変更とメンバーの削除は店舗管理者のみ行えます。</Muted>}
        </ScrollView>
      </KeyboardAvoidingView>
