...
export type ExportInput = {
  ...
  /**
   * 経費と月別利益も書き出すか。**既定は false。**
   *
   * ⚠️ **CSV では 1 つのファイルに 3 つの表が縦に並ぶ**（集金 / 経費 / 月別利益）。
   *    iOS の出口は共有シートしか無く**返せるファイルは 1 つ**だから。
   *    そのぶん**会計ソフトへそのまま取り込める形ではなくなる**ので、
   *    既定を変えず利用者に選ばせる。分けて読みたいときは Excel（シートが分かれる）。
   * ⚠️ **店舗を絞ると組織全体の経費（`laundry_id` が NULL）は入らない。**
   *    按分の規則が無いので勝手に割らない、という規約（店舗別の月別利益と同じ）。
   */
