...
- ⚠️ **`revenue/_layout.tsx` の `fullScreenGestureEnabled: false` を外さない。**
...
- **経費の入口はこのカードの中に置く**（アプリ、2026-08-03）。棒が「売上 − 経費」
  なので、経費を足したくなるのはここを見た瞬間。**「一覧へ」と「追加」の 2 つ**を並べる。
  ...
  - ⚠️ **店舗別のページからは `storeId` を載せて追加画面へ渡す。** 載せないと、
