...
- `role`: `"admin" | "collecter" | "viewer"` — ⚠️ **`collector` ではなく `collecter`**
  - ⚠️ **役割は 3 つ。`admin か、それ以外か` の二分岐で表示名を出さないこと。**
    `role === "admin" ? "店舗管理者" : "集金担当者"` と書くと**閲覧者が
    「集金担当者」と表示される**（初期設定の確認画面と組織参加の完了画面が
    実際にそうなっていた）。表示名は表から引く（`SETUP_ROLE_LABEL` / `ROLE_LABEL`）
  - ⚠️ **分岐は `role === "admin"` で書く。** `role === "collecter"` で
    「非管理者」を表すと viewer が漏れる
  - ⚠️ 初期設定は 2026-08-05 まで **`admin` と `collecter` の 2 択**だった。
    サーバ（`POST /profile`）は最初から 3 つとも受け付けている
...
- ⚠️ **未読の下限は「既読の線」と「アカウントを作った時刻」の新しいほう**
  （`unreadSince()`、2026-08-05）。既読の線は新規登録の直後だと必ず 0 なので、
  そのままだと**使い始める前の告知まで全部未読**になり、初めて開いた画面に
  いきなりバッジが付く。
  - ⚠️ **Web にも同じ仕組みがある**（既読の線が localStorage なだけで考え方は同じ）。
    **片方だけ直すと、同じ人が Web とアプリで違う未読件数を見る。**
    判定は 2 リポジトリの `unreadSince()`（アプリ `announcementsRead.ts` /
    Web `src/functions/announcements.js`）
  - 登録時刻の取り方が**アプリと Web で違う**:

    | | 取り方 |
    |---|---|
    | アプリ | `GET /bootstrap` の **`user.createdAt`（epoch ミリ秒）** |
    | Web | サーバコンポーネントの `getUser()` の **`created_at`（ISO 文字列）** |

    ⚠️ **アプリへはサーバで epoch に畳んでから返す。** ISO 文字列を渡すと端末で
    `new Date(<文字列>)` を通すことになり Hermes のパースに寄りかかる
    （日付が全画面 NaN になった事故と同じ入口）。Web は V8 なので ISO のままでよく、
    `unreadSince` は `toEpochMs` を通すので**どちらが来ても同じ数値になる**
  - ⚠️ **`undefined` / `null` になり得る**（この項目を返す前の応答が永続キャッシュから
    復元される／サーバが時刻を読めなかった）。そのときは **0 に倒す＝これまでどおり
    全部未読**。⚠️ **「今」に倒さないこと。** 倒すと、値が無い間に公開された
    お知らせが**永久に未読にならない**
  - ⚠️ **判定は `unreadSince()` の 1 か所に集約する。** バッジ（設定画面）と
    「新着」の印（お知らせ一覧）で別々に書くと、**バッジは 0 なのに一覧には
    新着の印が残る**というずれ方をする
...
| 組織名 | `PATCH /org` |
| 参加申請の承認・却下 | `POST /org/join-requests/[id]` |
...
- `POST /org/join`（参加の申請）… ⚠️ **まだメンバーになっていない。**
  記録すると `org_id` が null の行になり、**組織のログに出ない行だけが増える。**
  記録は承認側（`POST /org/join-requests/[id]`）で行う
...
- ⚠️ **メールアドレス**（参加の申請先として入力された管理者のもの）

⚠️ **判断の基準は「その情報が見える範囲と、ログが読まれる範囲が合っているか」。**
メール招待の宛先を一度ログに残していて外したのがこれで、**保留中の招待一覧は
admin にしか出していないのに、ログは組織の全員が読む**ため、そこだけ可視範囲が
広がっていた。⚠️ **招待そのものは 013（2026-08-06）で廃止済み**だが、
**同じ形の間違いは新しい機能でも起こる。**
...
| 文書 | 正本（Web） | アプリ |
|---|---|---|
| 利用規約 | `../coin-laundry-app/src/app/app/terms/page.jsx` | `src/content/legal/terms.ts`（**複製**） |
| プライバシー | `../coin-laundry-app/src/app/privacy/page.jsx` | `src/content/legal/privacy.ts`（**複製**） |
| 特商法 | `../coin-laundry-app/src/app/tokushoho/page.jsx` | `src/content/legal/tokushoho.ts`（⚠️ **別物**） |

**利用規約とプライバシーは片方だけ直すと、アプリだけが古い規約を出し続ける。
必ず両方を同時に直すこと。**

⚠️ **特商法だけは複製ではない。** Web 版は Stripe 決済（当方が販売者）、
アプリ版は App Store 経由（Apple が販売者）と**前提が違うので中身が異なる。**
**片方を他方に写さないこと。** 揃える必要があるのは事業者情報だけ。
...
- ⚠️ **特商法はアプリ専用に書き下ろした版を載せている**（`src/content/legal/tokushoho.ts`、
  2026-08-05）。⚠️ **Web の `/tokushoho` を写経しないこと。** あちらは Stripe 決済
  （当方が販売者）を前提に販売価格と決済条件を書いているので、そのまま載せると
  **外部での購入条件の提示**になる（3.1.3(a)）
  - ⚠️ **「アプリ内課金が無いから掲示義務が無い」という理由は 2026-07-29 に失効**
    （IAP を入れたため）。載せない選択も可能だったが、**日本限定配信にしたことで
    価格を正しく書けるようになった**ため載せる側に倒した
  - ⚠️ **返金を「当方が行う」と書かない**（IAP の返金は Apple の手続きで、開発者は
    実行できない）。「Apple 所定の手続きに従ってご請求ください」に留める
  - ⚠️ **メール・電話をリンクにしない**（`mailto:` / `tel:` はアプリ外への導線）。
    `contact` ブロックは文字列を描くだけなので、押せるようにしないこと

### ⚠️ 配信地域が日本のみであることが前提になった（2026-08-05）

**`tokushoho.ts` に販売価格を円で固定している。** これは `products.ts` の
「価格を文字列で持たない」規約の**唯一の例外**で、特商法が販売価格の記載を
要求するため意図的にそうしている。

- ⚠️ **配信地域を日本以外へ広げた瞬間、表示額と請求額が食い違う**（3.1.2）。
  広げるなら価格の記載を外すか、地域ごとに出し分ける必要がある
- ⚠️ **価格改定のたびに `tokushoho.ts` を直してビルドし直す。** ASC の価格は
  即座に反映されるが、こちらは**審査を通るまで反映されない。**
...
- **アプリ内で価格を数字で書いてよいのは `tokushoho.ts` だけ。** 他は
  `displayPrice` のみ（`grep -rE "月額[0-9,]+円|¥[0-9,]{3,}"` で確認できる。
  他のファイルに出るのは ⚠️ コメントだけであること）
...
- ⚠️ **ホームの月別カードは「集金のある一番新しい月」から開く**
  （`initialMonthIndex()`、2026-08-05）。当月固定にすると、当月にまだ集金が無い組織で
  **開いた瞬間に ¥0 のカードが出て「売上が表示されない」ように見える**（実際にそう見えていた）。
  - 当月に集金があれば当月のまま＝**ふだん使いの見え方は変えない**
  - 1 件も無い組織は当月（¥0 で正しい）
  - ⚠️ **`useCardSwipe` に渡すのは数値**。配列や関数を渡すと、react-query が
    画面復帰で取り直すたびに**指で送った位置が当月へ引き戻される**
...
```
{ format: "csv" | "xlsx", startEpoch, endEpoch, storeIds?, splitMethod?, includeExpenses? }
  → { data: { format, name, base64, recordCount, expenseCount } }
```

### 経費と月別利益を足す（`includeExpenses`、2026-08-03）

⚠️ **既定は false。** 足すと**表が 3 つ**になる（集金 / 経費 / 月別利益）。

**経費は集金データと同じシートに、同じ区別で入る**（2026-08-03）。
...
- ⚠️ **グループのキーを `groupRecords` と一致させる**（`groupExpenses`）。
  ずれると同じ月・同じ店舗なのに別のシートへ分かれる
- ⚠️ **集金の表に経費の「列」を混ぜない。** あちらは
  **「設備 + 現金（内訳なし）+ 支払方法 = 合計」が常に成り立つ**表で、
  確定申告の材料に使われる。行の意味（1 行 = 1 回の集金）も違う。
  **空行 + 「■ 経費」の見出しで区切った別ブロック**にする
  （空行が無いと Excel が前の表の続きとして読んで列がずれる）
- ⚠️ **それでも同じ列に金額が縦に並ぶ。** 列をまるごと選択して合計すると
  **集金と経費が混ざる。** ブロックを分けても消せない性質
- ⚠️ **日付は `epochToDateStr` を使う**（集金と同じ関数）。
  同じシートに縦に並ぶので、自前で組むと**同じ列に 2 種類の書式が混ざる**
- ⚠️ **どのシートにも入らない経費を黙って落とさない。** 別シートへ逃がす。
  該当するのは 2 つで、**どちらも実際に起こる**:
  - `"store"` で分けたときの**組織全体**（`laundry_id` が NULL）。
    店舗に紐づかないので、どれかに入れると 1 店舗の数字に税理士費用が乗り、
    全部に入れると重複する
  - **集金の無い月・店舗の経費**（`groupRecords` は集金からしかグループを作らない）
- ⚠️ **月別利益だけは 1 枚独立。** 期間をまたぐ表なので、月ごとのシートに入れると
  1 行しか出ず比べられない
- ⚠️ **数値の列の位置が 3 つとも違う。** 集金は 0/1/最終列が文字列、
  経費は最終列だけが金額、月別利益は最終列（利益率）だけが文字列。
  **判定を使い回さないこと**
- ⚠️ **CSV は 1 つのファイルに 3 つの表を縦に並べる**（返せるファイルは 1 つなので）。
  **したがって「1 ファイル = 1 表」ではなくなり、会計ソフトへそのまま
  取り込めなくなる。** だから既定を変えず利用者に選ばせ、画面でも注意を出す。
  分けて読みたい人には Excel を勧めること（あちらはシートが分かれる）
  - ⚠️ **区切りは空行 + 見出し行。** 空行が無いと Excel が前の表の続きとして
    読んで列がずれる
- ⚠️ **利益は負になり得る。** `Math.max(値, 0)` で潰さない（**赤字の月が黒字に見える**）
- ⚠️ **利益率は売上が 0 のとき空欄。** 0 で割ると `Infinity%` が表に出る。
  経費だけ入れて集金がまだ無い月は普通に起こる
- ⚠️ **合計行の利益率は「利益の合計 ÷ 売上の合計」で出し直す。** 月ごとの率を
  平均すると、売上の小さい月が同じ重みで効いて実態とずれる
- ⚠️ **集金が 0 件の月でも経費があれば行を出す。** 出さないとその月の赤字が表から消える
- ⚠️ **売上は `totalFunds`（総額）。** 現金だけの額ではない
- ⚠️ **店舗を絞ったら、組織全体の経費（`laundry_id` が NULL）は含めない。**
  按分の規則が無いので勝手に割らない（店舗別の月別利益と同じ規約）。
  残すと「1 店舗ぶんの書き出し」に税理士費用が丸ごと乗る
- ⚠️ **経費は担当店舗（011）でサーバが絞る。** 担当外のぶんは書き出しにも入らない
- ⚠️ **`expenseCount` の 0 と null を区別する。** 0 =「含めたが 1 件も無かった」、
  null =「含めていない」。まとめると、選んだのに 0 件だったときに
  **失敗したのか本当に無いのか分からなくなる**
- ⚠️ **整形は `src/functions/expenseExport.js`**（Web）。⚠️ **アプリに持たせない**
  （2 リポジトリに同じ整形を置くと、片方だけ直したときに型エラーも出ないまま列がずれる）
...
⚠️ **それまで経費は担当店舗（011）で絞っておらず、役割の判定も
「viewer でなければ何でもできる」だった。** 両方を絞った。

| | admin | 集金担当者 | 閲覧者 |
|---|---|---|---|
| 一覧を見る | 全店舗 | **担当店舗 + 組織全体** | **担当店舗 + 組織全体** |
| 単発の登録 | ○ | ○（担当店舗と組織全体だけ） | × |
| 単発の**編集** | ○ | **自分が登録した当月の分だけ** | × |
| 単発の**削除** | ○ | **×** | × |
| 固定費の追加・編集・削除 | ○ | **×** | × |

⚠️ **編集と削除で条件が違う。** 直せる範囲は広げたが、消す操作は広げていない。
`requireAdmin` を単発の編集に使い回さないこと。

- ⚠️ **組織全体（`laundry_id` が NULL）は誰にも残す。** 店舗の経費ではないので
  「担当していない店舗」に当たらない。落とすと**税理士費用のような組織の支出が
  admin 以外に一切見えなくなる**うえ、担当 0 件の人の画面が完全な空になる
- ⚠️ **絞るのは SQL 側**（`applyStoreScope`）。取ってきてから手元で捨てると、
  **1000 行の上限が捨てるぶんで埋まって**担当店舗の古い経費が黙って欠ける
- ⚠️ **`storeIds === null` が「全店舗」**（admin）。空配列は「担当 0 件」で意味が違う。
  判定は `src/functions/expenseScope.js` の `expenseScopeFilter` / `inExpenseStoreScope`
  を通す（⚠️ `"use server"` の `expenses/action.js` には**同期の関数を置けない**）
- ⚠️ **`?storeId=` で名指しされたら担当を確認して 403。** 一覧に出ない店舗でも
  URL を手で組めば覗ける（`getStore(id)` と同じ穴）
- ⚠️ **書き込み先も両方見る。** 「組織の店舗か」だけだと担当外の店舗に経費を作れて、
  **作った本人が二度と見られない行**が残る
- ⚠️ **展開した固定費（`getExpenses`）と定義の一覧（`getRecurringExpenses`）を
  同じ範囲で絞る。** 片方だけだと、一覧に出ない家賃が「毎月の固定費」の欄にだけ並ぶ
#### 集金担当者の編集は「自分が登録した当月の分」だけ

⚠️ **可否は行ごとに変わる。** 同じ画面に**押せる行と押せない行が混ざる。**

- ⚠️ **判定はサーバがして、応答の `editable` に畳んで返す**
  （`canEditExpense`）。**アプリと Web で「自分か」「当月か」を組み立て直さないこと。**
  同じ規則が 2 リポジトリに並ぶと必ずずれる
- ⚠️ **`editable` は古い応答・永続キャッシュでは `undefined`。**
  受け側は **`editable ?? 管理者か`** に倒す（**true に倒さない**。倒すと
  押しても 403 になる導線が出る）
- ⚠️ **既存の行を読んでから判定する。** `created_by` と元の `date` は
  リクエストに入っていない（クライアントの申告を信じない）
- ⚠️ **移動先の月も見る。** 見ないと当月の行を**先月の日付に付け替えて**
  過去の月を動かせる（編集できる範囲から自分で抜け出せてしまう）
- ⚠️ **当月に限るのは、締めた過去の月の数字が後から動くのを防ぐため。**
  権限を戻したのではなく、「登録できるのに直せない」を埋めるための緩和
- ⚠️ **`created_by` が null の行は直せない。** 008 は `ON DELETE SET NULL` なので、
  登録した人が退会すると null になる。**閉じるほうへ倒す**
- ⚠️ **削除は今も admin だけ。** 集金担当者が間違えた行は
  **金額・カテゴリ・日付を直して使う**ことになる
- ⚠️ **固定費は追加だけでなく編集・削除も admin。** 追加だけ塞いでも、
  **編集で金額を書き換えれば同じことができる**
- ⚠️ **アプリの出し分けは 2 つに分ける**（`canAdd` と `isAdmin`）。1 つの `canEdit` に
  まとめると集金担当者に押しても 403 になるボタンが出る。Web も同じ
  （`ExpensesPanel` の `canAdd` / `canManage`）
- ⚠️ **集金担当者には追加の 2 択シートを出さない。** 固定費が admin 専用なので、
  片方が必ず 403 になる。単発の入力へ直行させる
...
#### 名前（`recurring_expenses.name`）は任意

2026-08-03 に**アプリの入力欄を外した**（「家賃」に「家賃」と名前を付けるだけの
欄になっていた。カテゴリが表示名を兼ねる）。

- ⚠️ **列は NOT NULL のまま。** マイグレーションはしていない。無ければ
  サーバがカテゴリで埋める（`recurringName`）。**null を入れると 23502 で落ちる**
- ⚠️ **Web の入力欄は残してある**（`RecurringPanel.jsx`）。送られた名前は
  サーバが尊重するので、**Web で付けた名前は消えない。**
  片方だけ見て「名前は使っていない」と判断しないこと
- ⚠️ **編集で名前を空にして送らない。** アプリの「今月で終わらせる」は
  既存の値をそのまま返している。空で上書きすると Web で付けた名前が消える
- ⚠️ 一覧の行では、**`note` がカテゴリと同じなら出さない**（同じ語が 2 回並ぶ）

#### 画面は 1 枚（`app/(tabs)/expenses/`）

⚠️ **2026-08-03 にタブへ昇格した**（それまでは `revenue/expenses/`）。
毎日触る記録なのに 収益 → 経費 の 2 タップが要ったため、**設定タブと入れ替えた。**
設定の入口は**ホームのヘッダ右上の歯車**（`GreetingHeader` の `onOpenSettings`）。

- ⚠️ **`/revenue/expenses` に戻さない。** 戻すと入口が 2 つになるか、
  同じ画面をタブと収益の 2 か所に置くことになる
- ⚠️ **ホームから歯車を消さない。** 設定への入口はそこだけで、
  **サインアウト・組織への参加・プラン・通知**がすべてその奥にある。
  ⚠️ **組織未所属のときはタブがホーム 1 本だけ**なので、`hasOrg` で隠さないこと
- ⚠️ **経費タブは `expensesEnabled` が false の組織では出さない**（`href: null`）。
  ⚠️ ただし**画面は残る**（設定で切った直後にそこに留まっていることがある）。
  判定は必ず `expensesEnabled()` を通す（永続キャッシュで `undefined` になり得る）

2026-08-03 に `expenses/recurring` の専用ページを畳んで経費画面の中へ移した。
同じ「経費」なのに固定費だけ行き先が違ったため。

- 上の一覧に出るのは**展開された結果**、下の「毎月の固定費」に並ぶのは**定義**。
  ⚠️ 同じものが 2 か所に見えるので、見出しで区別が付くようにしてある
- **追加は 1 つのボタンから 2 択**（単発 / 固定費）。⚠️ **まとめない**
  （入力項目が違ううえ、固定費は「毎月ずっと計上される」という別の意味を持つ）
- ⚠️ **選択のあとは `onDismiss` を待ってから動く。** 単発は `router.push` で
  画面へ行くので、シートを開いたまま push すると**以降タブを切り替えても
  何も表示されなくなる**（`docs/traps.md` の iOS）

### 記録先は「店舗ごと」と「組織全体」の 2 通り

`laundry_id` が **NULL = 組織全体**（税理士費用のように店舗に紐づかない支出）。
単発・固定費のどちらにもあり、Web とアプリの両方のフォームで選べる。

- ⚠️ **`getExpenses(start, end, laundryId)` に店舗を渡すと「その店舗のものだけ」**。
  **組織全体（NULL）の行は入らない。** 按分の規則が無いので勝手に割らない、
  という判断。⚠️ したがって**店舗別の月別利益は組織全体の経費を引いていない。**
  カードにその旨を出してある（出さないと経費画面の合計と合わない理由が分からない）
- ⚠️ **「組織全体の経費だけ」を取る手段は API に無い。** 絞り込みは
  取ってきた月ぶんを**手元で分ける**（アプリの `ExpenseScopeFilter`、
  Web の `ExpensesPanel`）。
  ⚠️ **`"all"`（絞らない）と `"org"`（NULL の行だけ）を 1 つの値で兼ねない。**
  兼ねると組織全体の経費だけを見る手段が消える
- ⚠️ **一覧の行に必ず対象を出す**（`◯◯店` / `組織全体`）。同じ金額・同じ
  カテゴリの行が店舗の数だけ並ぶので、無いとどれがどれか区別が付かない
- ⚠️ **店名は応答の `laundryName` を使う。店舗一覧から引かない。**
  `getStores()` は担当店舗（011）で絞るので、集金担当者・閲覧者では
  **担当外の店舗が落ちる。** 一覧から引くと**引けなかった店舗が全部
  「（削除された店舗）」になる**（2026-08-03 に実際にそうなっていた）。
  ⚠️ **経費を担当店舗で絞ったあとも、店名を引く側は組織の全店舗のまま**にすること
  （`getOrgStoreNames`）。絞った側で引き直すと同じ穴が空く。
  - サーバは `getOrgStoreNames()`（**service client で組織の全店舗**）から焼く。
    ⚠️ **`getStores()` で作らないこと**（同じ穴が空く）
  - `laundryName` が **`null` = サーバが探して見つからなかった＝本当に消えている**、
    **`undefined` = 古い応答**（永続キャッシュ）。⚠️ **後者で「削除された」と
    言い切らない。**「他の店舗」に倒す（消えたのか担当外なのか区別が付かない）
  - 判定は 1 か所に集約してある（アプリ `scopeLabel`、Web `expenseTargetName`）
- ⚠️ **絞り込んだら合計とカテゴリ内訳も追従させる。** 全体の合計のまま残すと
  数字と行が食い違う

### 月は横スライドで送る

**`ChartPager`**（月別売上と同じもの）で送る。縦に積むと 12 か月ぶんが
縦一列になり、下の「毎月の固定費」まで遠くなるため 2026-08-03 に変えた。

- ⚠️ **`ScrollView` で作り直さない**（`docs/traps.md` の react-native-web）
- ⚠️ **`expenses/_layout.tsx` の `fullScreenGestureEnabled: false` を外さない。**
  iOS 26 以降は画面のどこから払っても戻るので、**送りが全部
  「前の画面へ戻る」に化ける**
- ⚠️ **矢印を必ず併設する。** 払う操作だけだと気づかれず、
  **VoiceOver から操作できない**
- ⚠️ **前後の月も先に取る。** 取らないと、指で引いている最中に隣の月が
  空で描かれ、送りきった瞬間に中身が現れる。**月ごとに 1 本ずつ引けば**
  react-query のキャッシュがそのまま前後の先読みになる（期間をまとめて
  引いて手元で分ける必要は無い）
- ⚠️ **未来の月は送らせない**（`canNext`）。空の月を無限にめくれてしまう
- ⚠️ **過去には下限を設けない。** 「最初の経費がいつか」を返す API が無く、
  集金の最古月で代用すると**それより前に入れた経費に辿り着けなくなる**
- ⚠️ **月の名前は送りの見出しに 1 か所だけ置く。** 面の中にも置くと、
  指で引いている最中に同じ月名が 2 つ並んで見える

### カテゴリ別の円グラフ

⚠️ **月ごとに 1 枚出す**（2026-08-03）。期間をまとめて 1 枚にすると
「先月は修繕費が重かった」のような**月の癖が平均に溶けて見えなくなる。**

- ⚠️ **月の中の並びを触らない。** サーバが実体と展開した固定費を混ぜてから
  日付の新しい順に並べている
- ⚠️ **合計を出すのは円のすぐ下の 1 か所だけ。** 見出しにも出すと同じ数字が
  数センチ離れて 2 回並ぶ
  - ⚠️ **ドーナツの穴に入れない**（2026-08-06 に外へ出した）。**穴より数字の
    ほうが長くなる。** 穴はアプリ 78px / Web 約 124px で、`¥1,234,567` は
    どちらもはみ出す（アプリは `adjustsFontSizeToFit` が縮め続けて読めなくなり、
    Web は溢れる）
  - ⚠️ **穴を広げて解決しない。** 輪が細ると扇の色の差が読めなくなる。
    **12 カテゴリを色だけで見分ける図**なので、輪の太さが情報量そのもの
  - ⚠️ **Web とアプリの両方にある**（`ExpenseCategoryDonut.jsx` /
    `ExpenseCategoryPie.tsx`）。片方だけ直すと同じ画面の形が食い違う

⚠️ **`react-native-svg` は入れていない。** ネイティブモジュールなので
**EAS の再ビルドが要る**（`docs/traps.md` の EAS）。既存のグラフと同じく
素の View だけで描く。

- 組み立ては `src/components/expenses/categoryPie.ts`、見た目は
  `ExpenseCategoryPie.tsx`。⚠️ **ファイル名を大文字小文字だけで分けない**
  （Windows の tsc が同一視して TS1149。`historyRows.ts` と同じ罠）
- ⚠️ **1 枚の扇は 180 度まで。** 半円を回して見せる作りなので、超える分は
  `pieChunks()` が分割する。**描く側に分岐を持たせない**
- ⚠️ **角度は割合ではなく累計金額から出し、最後の 1 枚は必ず 360 度で終わらせる。**
  各片を丸めてから足すとずれが積もり、**円の縁に背景色の隙間が入る**
- ⚠️ **金額が数値であることまで確かめる**（永続キャッシュの罠）。0 円・負は数えない

### 経費を使うかは組織ごと（012）

`organizations.expenses_enabled`。初期設定で聞き、**設定 → 組織**で変えられる。

- ⚠️ **表示の設定であって認可ではない。** false でも `expenses` /
  `recurring_expenses` の行は消さないし、**API も 403 にしない。**
  戻したときに以前の記録がそのまま出るのが正しい挙動で、途中で切られた端末が
  永久に 403 を受け続ける事故も防げる。**プランの制限と混同しないこと**
- ⚠️ **既定は true。** false を既定にすると、**アップデートした瞬間に
  入力済みの経費が画面から消える**
- ⚠️ **判定は必ず `!== false`。** react-query の永続キャッシュ（MMKV）から
  この項目を持たない応答が最大 7 日復元されるので `undefined` になり得る。
  `Boolean(...)` で畳むと**アップデート直後だけ経費が消える。**
  アプリは `expensesEnabled()`（`src/components/expenses/expensesEnabled.ts`）を
  必ず通す（出し分けが 5 か所以上あり、書き方が混ざると画面ごとに挙動が変わる）
- ⚠️ **変更できるのは admin だけ**（組織全員の画面が変わるため）。
  ⚠️ 組織名の変更は `owner_id` で絞っている（オーナーのみ）ので**条件が違う。**
  揃っていないのは意図的
- ⚠️ **切ると「月別利益」のタブごと消える。** 選択が `"profit"` のまま残ると
  何も描かれないカードになるので、**表示側で「月別」へ倒す**
  （Web もアプリも `activeTab` で同じことをしている）
- ⚠️ **経費のボタン・FAB・固定費の入口も一緒に消す。** アプリは FAB を
  条件の外に置いていて「＋」だけが浮いて残っていた
- アクションログには**残す**（個人に閉じた設定ではなく組織の設定のため）

## 月別利益（売上 − 経費）

収益ページの 3 枚目のタブ。**2026-08-03 に「月次サマリー」と差し替えた**
（前月比・前年同月比の表より、売上 − 経費のほうが日々の判断に効くため）。

- ⚠️ **利益は負になり得る。** `Math.max(値, 0)` で潰さないこと。
  **赤字の月が黒字に見える。** 棒は 0 の線をまたいで下へ伸ばす
- ⚠️ **0 の基準線は必ず描く。** 無いと下に伸びた棒の意味が読めない
- ⚠️ **利益率は売上が 0 のとき null。** 0 で割ると `Infinity%` が画面に出る。
  経費だけ入れて集金がまだ無い月は普通に起こる
- ⚠️ **売上は `/funds/chart` の `total`（総額）を使う。** 現金だけの額ではない。
  キャッシュレスを落とすと利益が実際より小さく出る
- ⚠️ **展開された固定費（`recurring: true`）も経費に足す。** 除くと家賃を
  含まない「経費」になり、利益が実際より大きく出る
- ⚠️ **期間を送ると経費も前後ぶんを先読みする。** 取らないと、指で引いている
  最中に見える隣の期間が「経費 0 円 ＝ 利益が売上と同じ」で描かれ、
  送りきった瞬間に棒が縮む
- ⚠️ **`getExpenses` の `to` は「含む」（`lte`）。** `/funds/chart` の `to` は
  排他なので**向きが逆**。翌月 1 日をそのまま渡すと 1 日ぶん混ざる
- ⚠️ **同じ組み立てが 2 リポジトリにある**（Web は
...
- **経費の一覧への入口はこのカードの中に置く**（アプリ、2026-08-03）。棒が
  「売上 − 経費」なので、経費を見たくなるのはここを見た瞬間。
  ⚠️ **`MonthlyProfitCard` は店舗別の収益ページにも出る**ので、ここに置けば
  両方に一度で乗る。
  ⚠️ **「追加」をここに並べない**（一度置いて外した）。経費を足す入口は
  **店舗詳細の「経費を追加」**と**経費タブの「＋」**の 2 つ
  - ⚠️ **タブをまたぐ遷移**になる（収益 → 経費、店舗 → 経費）。
    経費タブの `backToTabRoot("/expenses")` を外さないこと。外すと、
    他タブから深い画面へ入ったときに**経費タブを押しても何も起きなくなる**
    （react-navigation の既定は「今いるタブを押しても何もしない」）
  - ⚠️ **店舗詳細からは `storeId` を載せて追加画面へ渡す。** 載せないと、
    その店舗を見ているのに「対象」を自分で選び直すことになる。
    ⚠️ **初期値であって固定ではない**（フォームで組織全体にも他店舗にも変えられる）
  - ⚠️ **viewer には「追加」を出さない。** サーバが 403 を返すので、
    出すと「押したのに失敗する」になる

## 組織への参加（013）

**従業員が申請 → オーナーが権限を選んで承認**の 1 本だけ（2026-08-06）。

⚠️ **それまでは 2 経路あった。どちらも畳んである。**

| 廃止したもの | なぜ |
|---|---|
| メール招待（`organization_invitations` + token 付きリンク） | 相手が未登録だと送れず、届かない・期限切れ・迷惑メール、と詰まりどころが多かった |
| 参加パスワード（`organizations.join_password`） | admin が一度配れば誰でも使える**無期限の合鍵**。しかも**即座にメンバーになれた** |

⚠️ **列とテーブルは残してある**（`join_password` / `organization_invitations`）。
**新しく参照するコードを書かないこと。** 書いた時点で合鍵が復活する。

| | |
|---|---|
| 申請 | `POST /org/join` に **`{ adminEmail }` だけ**。⚠️ `password` を受ける形に戻さない |
| 自分の申請 | `GET /org/join`（⚠️ 無ければ `null`。**エラーではない**） |
| 取り下げ | `DELETE /org/join` |
| 届いている申請 | `GET /org/join-requests`（⚠️ **admin 以外には空配列**。403 ではない） |
| 承認・却下 | `POST /org/join-requests/[id]` に `{ decision, role }` |

- ⚠️ **承認できるのは店舗管理者（admin）。** メンバーの権限変更・削除と同じ条件。
  - ⚠️ **オーナー（`organizations.owner_id`）限定にしない**（2026-08-06 に変えた）。
    **オーナーは組織を作った 1 人だけで、譲渡する手段が無い。** 限定すると
    **オーナー不在の間は誰も組織に参加できなくなる**
  - ⚠️ **オーナー限定なのは「組織名の変更」と「組織の削除」だけ。**
    `updateMemberRole` は `admin` への昇格を許すので、**admin は何人でも増える。**
    「admin ＝ オーナー」と考えないこと
- ⚠️ **権限は承認する側が承認時に選ぶ**（`collecter` / `viewer`）。
  申請には権限を持たせない。⚠️ **`admin` を選べるようにしない。**
  承認だけで管理者を増やせてしまう（昇格は `updateMemberRole` を通す）
- ⚠️ **メンバーが増えるのは承認だけ。** プランの人数制限もそこ 1 か所で見る
  （「プランの制限」を参照）
- ⚠️ **却下された申請は残す。** 部分ユニークを `WHERE status = 'pending'` に
  してあるので出し直せる。**全体ユニークにすると一度断られたら二度と申請できない**
- ⚠️ **承認後に他組織への pending を消す。** 1 人が複数の組織へ申請できるので、
  放っておくと**すでにメンバーなのに承認できる申請**が残り、別のオーナーが
  承認して失敗する（`organization_members` は 1 人 1 組織）
- ⚠️ **申請の一覧にメールアドレスを載せない。** 表示名で足りるうえ、
  連絡先を配る理由が無い（アクションログと同じ判断）
- ⚠️ **申請が届いたことはメールでしか伝わらない。** アプリにも Web にも出るが
  **見に行かないと気づけない。** `requestJoinOrg` が入力されたメール宛に送る
  （失敗しても申請は成立させる）
- ⚠️ **同じ画面が Web とアプリの両方にある**（`JoinOrgForm.jsx` /
  `app/join-organization.tsx`、`OrganizationSettings.jsx` /
  `JoinRequestSection.tsx`）。**片方だけ直すと同じ操作の手順が食い違う**
- ⚠️ **「参加が完了しました」と出さない。** 申請しただけではまだ入れていない

## 担当店舗（011）

メンバーごとに触れる店舗を絞る。**担当でない店舗の集金データ・在庫データには
一切かかわれない。**

| | |
|---|---|
| `member_stores` | 1 行 = 「この人はこの店舗を担当する」 |
| admin | ⚠️ **常に全店舗。行を持たない** |
| 行が 0 件 | ⚠️ **1 店舗も担当しない**（何も見えない） |

### ⚠️ 強制するのは `getStores()` の 1 か所だけ

集金（`collectFunds/action.js`）も在庫（`laundryState/action.js`）も、
**自前の `getOrgStoreIds()` から `getStores()` を呼んでいて、合計 20 か所以上が
そこを通る。** 個々の関数に判定を撒くと必ず撒き漏らすので入口で絞ってある。
**BFF は Server Action を包んでいるだけ**なので、Web とアプリの両方に同時に効く。

- ⚠️ **`laundry_store` を直接引く経路を新しく作らないこと。** 作った時点で
  そこだけ担当店舗を素通りする
- `getStores()` を通らない 2 つは自前で見ている:
  - `getStore(id)` … 一覧に出ない店舗を **URL 直打ちで開けてしまう**ため
  - `getLaundryState(laundryId)` … ⚠️ **2026-08-03 まで組織の確認すら無く、
    ログインさえしていれば他組織の在庫状態まで読めていた**
- ⚠️ **`storeIds === null` が「全店舗」。空配列とは意味が違う。**
  `storeIds?.length` のような書き方で判定しないこと
- ⚠️ **取得に失敗したら空に倒す**（開くほうへ倒さない）

### 割り当ての口

`PUT /api/v1/org/members/[userId]/stores`（`{ laundryIds }`）。**置き換えであって
部分更新ではない。**

- ⚠️ **admin だけが通る。** 開けると集金担当者が自分に店舗を割り当てられ、
  この機能そのものが意味を失う。`member_stores` に**書き込みの RLS ポリシーを
  作っていない**のも同じ理由（変更は Server Action が admin を確認してから
  service role で行う）
- ⚠️ **`laundryIds` はサーバで検証する。** そのまま入れると**他組織の店舗 id を
  割り当てられる**
- ⚠️ **admin には割り当てない**（400）。行を持たせると、店舗を追加するたびに
  admin ぶんを足す必要が生まれ、**足し忘れた瞬間に管理者が自分の店舗を見られなくなる**
- 一覧は `GET /org/members` に `storeIds` として乗っている。
  ⚠️ **専用の GET を足さないこと**（権限は変えたのに担当が古いまま、が生まれる）
- ⚠️ **`storeIds` は admin から見たときだけ入る。** admin 自身は常に空なので、
  画面では「未設定」ではなく**「全店舗」**と出すこと

### ⚠️ 運用で必ず踏むところ

- **新しく作った店舗は誰にも自動で配られない。** admin が割り当て直す。
  Web・アプリの両方の画面にその旨を出してある
- **admin から降格した人は担当 0 件になる**（何も見えない）。降格したら続けて割り当てる
- **担当 0 件と「組織に店舗が無い」はアプリから区別できない。** どちらも空配列。
  管理者は常に全店舗なので、**非管理者のときだけ「担当する店舗がありません」**と
  出し分けている。分けないと集金担当者は組織ごと空だと思って管理者に連絡しない
  - ⚠️ **文面と「店舗を登録する」ボタンは `NoStoresNotice` の 1 か所に集約してある**
    （2026-08-05。ホーム・店舗一覧・収益・管理の 4 画面が同じものを出す）。
    **画面ごとに書き写さないこと。** 4 か所に散ると、担当店舗の出し分けを
    直したときに必ず撃ち漏らす
  - ⚠️ **ボタンは admin にしか出さない。** `createStore` が `myRole !== "admin"` を
    弾くので、出すと**押しても 403 になるボタン**になる
  - ⚠️ **判定に「集金のある店舗」の一覧を使わない**（収益ページの `StoreRevenue` が
    これ）。店舗を登録済みで集金がまだ 0 件のときも 0 件になり、
    **「まず店舗を登録しましょう」が消えなくなる。** 見るのは `GET /stores`
- ⚠️ **店舗に紐づく通知は担当者と admin にだけ送る**（`pushToOrg` の `laundryId`）。
  送ると**開いても 403 になる画面へタップで誘導する**ことになる。
  ⚠️ 集金リマインダーの Edge Function は**別デプロイなので未対応**
- ⚠️ **経費（008）も 2026-08-03 に絞った。** ただし**強制は `getStores()` ではなく
  `expenses/action.js` の中**（`applyStoreScope`）。組織全体の経費
  （`laundry_id` が NULL）を残す必要があり、店舗 id の集合では表せないため。
  **経費を読む経路を新しく作るときは、そこにも同じ絞りを入れること。**
  規約は「経費（008）」の「担当店舗と権限」
...
- ⚠️ **メンバーが増える経路は `decideJoinRequest` の承認だけ**（013、2026-08-06）。
  それまでは 3 つ（招待を作る / 招待を受ける / 参加パスワードで入る）あり、
  2026-08-01 まで**どれも `plan` を見ておらず free でも無制限に増やせた。**
  経路を 1 本にしたので**上限を見る場所も 1 か所**になった。
  ⚠️ **「メンバーを追加する」経路を新しく作らないこと。** 作った時点で
  判定が 2 か所に分かれ、片方を直し忘れる（それが 2026-08-01 の事故そのもの）
- ⚠️ **申請の時点では見ない。承認の直前に見る。** 申請が溜まっている間に
...
- ⚠️ **判定するのは「増やすとき」だけ。** プランを下げても既存メンバーは外さない。
  ...
  **上限を超えたまま留まるのを許容する**
- ⚠️ **保留中の申請は数えていない。** 申請はいくつでも溜まるが、
  行が増えるのは承認のときだけなので上限には影響しない
...
**アカウントを作れる以上、アプリ内から完全に削除できなければならない。**
「退会」でも「Web で削除してください」でも要件を満たさない。

⚠️ **入口は 2 つ要る。** 2026-08-05 まで設定画面にしか無かった。

| 状態 | 入口 |
|---|---|
| 組織に所属している | ホーム右上の歯車 → 設定 → 最下部 |
| **組織未所属** | **`/join-organization` の最下部** |

⚠️ **組織未所属のユーザーは `app/index.tsx` の振り分けで `/join-organization` へ
リダイレクトされ、タブにもホームの歯車にも辿り着けない。** つまり
**組織に入っていない集金担当者・閲覧者は削除する手段が 1 つも無かった。**
「設定画面にあるから足りている」と考えないこと。

### ⚠️ 本人確認はサインイン方法で分ける

⚠️ **Apple でサインインした人はパスワードを持っていない。**
`signInWithPassword` だけで本人確認していた間、**Apple のユーザーは
「パスワードが違います」から先へ進めず、アカウントを永久に削除できなかった。**

- 判定は **`session.user.identities`** を見る。⚠️ `app_metadata.provider` は
  「主たる provider」1 つしか返さないので、**両方でサインインできる人を取り違える**
- `email` があればパスワード、**無くて `apple` があるときだけ** Apple で再認証
  （`signInAsync` → `signInWithIdToken`）
- ⚠️ **識別子が取れないときはパスワードに倒す**（Apple のシートを出しても
  通らない可能性がある。パスワード欄なら少なくとも「違う」と伝えられる）
- ⚠️ **本人確認を飛ばさないこと。** 端末を拾った人がアカウントを消せてしまう
