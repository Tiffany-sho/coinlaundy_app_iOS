...
| 届いている申請 | `GET /org/join-requests`（⚠️ **オーナー以外には空配列**。403 ではない） |
| 承認・却下 | `POST /org/join-requests/[id]` に `{ decision, role }` |

- ⚠️ **承認できるのはオーナー（`organizations.owner_id`）だけ。**
  メンバーの権限変更・削除は **admin 全員**なので**条件が違う。** 意図的
  （組織名の変更と同じ条件）。⚠️ **オーナーが不在の間は誰も参加できない**
- ⚠️ **`isAdmin` で承認 UI を出し分けないこと。** admin でもオーナーでなければ
  通らないので、**押しても失敗するボタン**になる。件数（空配列）で出し分ける
