...
**基準は「アプリで toast（または同等の完了表示）が出る操作」**（2026-07-31 の決定）。
...
| プロフィール（氏名・表示名 / アイコン / 集金方法） | `PATCH /profile` |
| 集金スケジュールの更新・未設定化 | `PUT /org/collect-schedule` |
| 通知設定 | `PATCH /notifications/prefs` |
...
⚠️ **文面に秘密を書かない。** 参加パスワード、招待の token（リンクそのもの）、
管理者のメールアドレスは残さない。ログは組織の全員が読む。

⚠️ **通知設定は個人の設定なのに組織全員のログに出る。** しかもトグル 1 つごとに
1 行増える。うるさければ最初に外す候補（他のログには影響しない）。
