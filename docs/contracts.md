...
⚠️ **月の行き来（前へ / 次へ）ではない。** 直近 N か月を出し、押すたびに
3 か月ずつ伸びる。古い経費に辿り着くまで何度も押すことになり、
**まとめて見返せなかった**ため 2026-08-03 に変えた。

- ⚠️ **期間は必ず切る**（1000 行の上限）。伸ばすのは利用者の操作のときだけ
- ⚠️ **複数月を出すので月の見出しが要る。** 無いと日付を読むまでどの月か分からない。
  見出しにはその月の合計も出す（画面上部の合計は期間全体のもの）
- ⚠️ **押しても scroll 位置を動かさない**（押した場所に留まりたい操作）
...
- 月でまとめるのは `buildMonthGroups()`（`monthGroups.ts`）。
  ⚠️ **経費が 0 件の月も返す。** 落とすと「さらに見る」を押しても画面が
  変わらないことがあり、**効いていないように見える。** 呼び出し側は
  そういう月をカード 1 行に細くして出す
- ⚠️ **月の中の並びを触らない。** サーバが実体と展開した固定費を混ぜてから
  日付の新しい順に並べている
- ⚠️ **合計はドーナツの中央にしか出さない。** 見出しにも出すと同じ数字が
  数センチ離れて 2 回並ぶ
- ⚠️ **今どの期間を見ているかを必ず出す。** 月カードだけだと、伸ばした先が
