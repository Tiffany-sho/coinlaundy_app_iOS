...
- ⚠️ **店名は応答の `laundryName` を使う。店舗一覧から引かない。**
  ...
  **担当外の店舗が落ちる。** 経費は担当店舗で絞っていないため、
  一覧から引くと**担当外の経費が全部「（削除された店舗）」になる**
  （2026-08-03 に実際にそうなっていた）。
...
- ⚠️ **経費（008）は絞っていない。** 「集金データ・在庫データ」を対象とした
  2026-08-03 の指示に沿った範囲。組織全体の経費（`laundry_id` が NULL）が
  あるため素直に絞れない。広げるかは要判断
