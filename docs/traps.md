...
- ⚠️ **型付きルート（`.expo/types/router.d.ts`）は dev server が動いたまま
  ...
  *is not assignable to parameter of type …* で落ちること。**コードは正しい。**
  ...
  ⚠️ **したがって順番はこう:** ソースの編集を全部終える → 再生成 → **すぐ落とす**
  → `tsc`。壊れていたら**編集をやめてから**やり直す。
...
## 未ログイン画面を廃止した（2026-08-01）

`app/welcome.tsx`（Netflix 型の紹介 3 枚）を消し、**未ログインの起点をログイン画面にした。**
「App Store で選んで入れた時点で説明は済んでいる」ため、1 タップ挟む価値が無かった。
決め手は背景のコラージュ 6 枚が**差し替え前提のモック画像のまま**だったこと
（出すなら実画面のスクショを作る作業が先に発生する）。

- ⚠️ **`/welcome` は起動時の入口だけでなく「サインアウト後の着地点」でもあった。**
  参照は 5 か所（起動時の振り分け / 設定のサインアウト / アカウント削除 /
  組織を抜けたとき / 初期設定の中断）。**入口だけ直すと残り 4 か所が死ぬ**
- ⚠️ **ログイン画面に「戻る」を出さない。** 起点なので戻り先が無い。
  `AuthScreen` は `onBack` を渡さなければボタンごと出さない作りにしてある
- ⚠️ **`useStatusBarStyle` は呼び出し元が無くなったが残してある。** 濃色の画面を
  足した瞬間に「離れても白文字が残る」罠が再発するため。消さないこと
- ⚠️ **審査でデモアカウントの重みが増した。** 起動して最初に出るのが素のログイン壁に
  なったので、**審査情報のアカウントが動かないと審査員は何も見られない**
  （ログイン必須自体は 5.1.1(i) 上は問題ない。全機能がアカウント前提のため）
- 復活させるなら `git show 4e42f48:app/welcome.tsx`。背景は `AuthBackground`、
  トークンは `onDark` / `authGradient`、白ボタンは `Button variant="light"` として
  同じコミットに入っている
