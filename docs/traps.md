...
- ⚠️ **「アプリ内課金が無い」を前提にした判断は 2026-07-29 に全部失効している。**
  ...
  - ⚠️ **特商法は 2026-08-05 にアプリへ載せた**（`src/content/legal/tokushoho.ts`）。
    「App Store 経由なら Apple が販売者だから載せなくてよい」という判断は**取り下げた。**
    載せない選択も可能だったが、**日本限定配信にしたことで価格を正しく書けるように
    なった**ため載せる側に倒している。⚠️ **Web の `/tokushoho` を写経しないこと**
    （あちらは Stripe 決済＝当方が販売者で前提が違う）。詳細は
